...
export const SettingsContent = ({ normalizedConfig, features, showUserProfile = false, userProfile, showFooter = true, footerText, customSections = [], showCloseButton = false, }) => {
    const tokens = useAppDesignTokens();
    const insets = useSafeAreaInsets();
    const { t } = useLocalization();
    const hasAnyFeatures = features.appearance ||
        features.language ||
    ...
    return (<ScrollView style={styles.scrollView} contentContainerStyle={[
      ...
      {customSections && customSections.length > 0 && (<>
          {Array.from(customSections)
                ...
                .map((section, index) => (<SettingsSection key={section.id || `custom-${index}`} title={section.title}>
                {section.content}
              </SettingsSection>))}
        </>)}

      {!hasAnyFeatures && (<View style={styles.emptyContainer}>
          <SettingsSection title={t("settings.noOptionsAvailable") || "No settings available"}>
            <View />
          </SettingsSection>
        </View>)}
